...
type EnCode = '32-bit-large' | '32-bit-small' | '16-bit-large' | '16-bit-small'

/**
 * @title  md5
 * @description  md5加密
 * @param {string} value 
 * @param {EnCode} [core='32-bit-small']
 * @returns {string}
 */
export function md5(value: string, core: EnCode = '32-bit-small'): string {
  const target = hex_md5(value)
  if (core === '32-bit-small') {
    return target
  }
  if (core === '32-bit-large') {
    return target.toUpperCase()
  }
  if (core === '16-bit-small') {
    return target.substring(8, 24)
  }
  if (core === '16-bit-large') {
    return target.substring(8, 24).toUpperCase()
  }
  return value
}
